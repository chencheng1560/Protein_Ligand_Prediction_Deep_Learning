...
if __name__ == '__main__':
    ...
    for i in range(base_index, validate_smaples + base_index):
        ...
        for k in range(base_index, validate_smaples + base_index):
            ...
            if(dist < filter_distance_threshold):
                ...
                #new_complex = np.expand_dims(new_complex, 0)
                complexes.append(new_complex)
        # predict
        complexes = np.array(complexes)
        #complexes = np.transpose(complexes)
        complexes = complexes.transpose([0, 2, 3, 4, 1])
    ...
    np.savetxt('test_predictions_of' + model_file + 'example.txt', predictions_top10, fmt='%i', delimiter='\t')
